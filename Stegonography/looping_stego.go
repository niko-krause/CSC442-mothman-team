...
NOTICE (2): the interval and offset values are unnecessary now, as the program loops through its own set in powers of 2.
*/
...
func retrieve(bitFlag bool, byteFlag bool, offset int, interval int, wrapper string){
	...
	for i :=0; i<11; i++ {
		interval = int(math.Pow(2, float64(i)))

		for o:=0; o<11; o++ {
			offset = int(math.Pow(2, float64(o)))

			//fmt.Println(interval, " ", offset)
			...
			if (byteFlag) {
				...
				for wIndex < len(wBytes) {
					...
					for j := 0; j<8 && wIndex < len(wBytes); j++{  //for every bit in the expected hidden file's byte (with respect to the limits of the wrapper)
